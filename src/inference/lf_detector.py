...
class ObjectDetector(object):
    ...
    @staticmethod
    def lf_retrive_belief_map(net_model,in_img, in_lf_img):
        if in_img is None:
            return []

        # Run network inference
        CV_image_tensor = transform(in_img)
        CV_image_torch = Variable(CV_image_tensor).cuda().unsqueeze(0)
        image_tensor = transform(in_lf_img)
        image_torch = Variable(image_tensor).cuda().unsqueeze(0)
        out, seg = net_model(image_torch)
        beliefs = out[-1][0]
